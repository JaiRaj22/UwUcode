...
expowt cwass FiweWatcha extends WatchewSewvice {
	...
	pwivate stawtWatching(): void {
		const cwient = this._wegista(new Cwient(
			...
			{
				sewvewName: 'Fiwe Watcha (pawcew, node.js)',
				awgs: ['--type=watchewSewvicePawcew'],
				env: {
					VSCODE_AMD_ENTWYPOINT: 'vs/pwatfowm/fiwes/node/watcha/pawcew/watchewApp',
	...
	ewwow(message: stwing) {
		this.onWogMessage({ type: 'ewwow', message: `[Fiwe Watcha (pawcew)] ${message}` });
	}
